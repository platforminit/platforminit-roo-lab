...
BASE="/srv/ch01/baseline/state-latest.json"
[ -f "$BASE" ] || { echo "FAIL: missing $BASE"; exit 1; }

mkdir -p /srv/ch01/reports

LATEST_JSON="$(
  /srv/ch01/baseline/capture-state-v2.sh >/dev/null
  ls -1t /srv/ch01/baseline/state-*.json | head -n1
)"
...
cmp_field() {
  ...
  new="$(jq -r "$jqexpr" "$LATEST_JSON" 2>/dev/null || true)"
